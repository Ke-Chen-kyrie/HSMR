...
class HSMRPipeline(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        with profiler.measure('06_hsmr.cpu_adapt_batch') if profiler is not None else nullcontext():
            batch = self.adapt_batch(batch)

        # 1. Main parts forward pass.
        with profiler.measure('06_hsmr.cpu_to_gpu_patches', cuda=str(self.device).startswith('cuda')) if profiler is not None else nullcontext():
            img_patch = to_tensor(batch['img_patch'], self.device)  # (B, C, H, W)
        outputs = self.forward_step(img_patch)  # {...}

        # 2. Prepare the secondary products
        # 2.1. Body model outputs.
        pd_skel_params = HSMRPipeline._adapt_skel_params(outputs['pd_params'])#outputs['pd_params']
        with profiler.measure('06_hsmr.gpu_body_model_skin_and_joints', cuda=str(self.device).startswith('cuda')) if profiler is not None else nullcontext():
            skel_outputs = self.skel_model(**pd_skel_params, skelmesh=False)
        pd_kp3d = skel_outputs.joints  # (B, Q=44, 3)
        with profiler.measure('06_hsmr.gpu_to_cpu_internal_skin', cuda=str(self.device).startswith('cuda')) if profiler is not None else nullcontext():
            pd_skin_verts = skel_outputs.skin_verts.detach().cpu().clone()  # (B, V=6890, 3)
        # 2.2. Reproject the 3D joints to 2D plain.
        with profiler.measure('06_hsmr.gpu_joint_reprojection', cuda=str(self.device).startswith('cuda')) if profiler is not None else nullcontext():
            pd_kp2d = perspective_projection(
    ...
    def forward_step(self, x:torch.Tensor):
        ...
        with profiler.measure('06_hsmr.gpu_vit_backbone', cuda=is_cuda) if profiler is not None else nullcontext():
            with PM.time_monitor('Backbone Forward'):
                feats = self.backbone(x[:, :, :, 32:-32])#torch.Size([4, 3, 256, 256]) -> torch.Size([4, 1280, 16, 12])
        # GPU_monitor.snapshot('HSMR forward backbone')


        # 2. Run the head to predict the body model parameters.
        with profiler.measure('06_hsmr.gpu_parameter_head', cuda=is_cuda) if profiler is not None else nullcontext():
            with PM.time_monitor('Predict Head Forward'):
                pd_params, pd_cam = self.head(feats)#pd_params['poses'].shape=torch.Size([4, 46]) pd_params['poses_orient'].shape=torch.Size([4, 3]) pd_params['betas'].shape=torch.Size([4, 10]) pd_params['poses_body'].shape=torch.Size([4, 43]) pd_cam.shape=torch.Size([4, 3])
        # GPU_monitor.snapshot('HSMR forward head')

        # 3. Transform the camera parameters to camera translation.
        with profiler.measure('06_hsmr.gpu_camera_conversion', cuda=is_cuda) if profiler is not None else nullcontext():
