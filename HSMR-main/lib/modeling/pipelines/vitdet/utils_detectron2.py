...
class DefaultPredictor_Lazy:
    ...
    def __call__(self, imgs):
        ...
        with torch.no_grad():
            inputs = []
            downsample_ratios = []
            with self.profiler.measure('03_detector.cpu_resize_augment_tensorize') if self.profiler is not None else nullcontext():
                for img in imgs:
            ...
            while sid < N_imgs:
                ...
                try:
                    batch_inputs = inputs[sid:eid]
                    if self.device.startswith('cuda'):
                        with self.profiler.measure('03_detector.cpu_to_gpu_images', cuda=True) if self.profiler is not None else nullcontext():
                            batch_inputs = [
                                {
                                    **model_input,
                                    'image': model_input['image'].to(self.device),
                                }
                                for model_input in batch_inputs
                            ]
                    with self.profiler.measure('03_detector.gpu_inference_and_postprocess', cuda=self.device.startswith('cuda')) if self.profiler is not None else nullcontext():
                        amp_context = (
                        ...
                        with amp_context:
                            preds_round = self.model(batch_inputs)
                    ...
                    if cur_bs > 1:
                        ...
                        and oom_retries.get(sid, 0) < 1
                    ):
                        oom_retries[sid] = oom_retries.get(sid, 0) + 1
                        torch.cuda.empty_cache()
                        get_logger(brief=True).warning(
                            f'CUDA OOM on image No.{sid}; cache cleared, retrying once.'
                        )
                    else:
                    ...
                    continue
                # Save the results.
                with self.profiler.measure('03_detector.gpu_to_cpu_predictions', cuda=self.device.startswith('cuda')) if self.profiler is not None else nullcontext():
                    preds.extend([{
