...
def parse_args():
    ...
    parser.add_argument('--profile', action='store_true', help='Record detailed CPU, transfer, CUDA, rendering, and output timings.')
    parser.add_argument('--profile_output', type=str, default=None, help='Profile path prefix. Defaults to <output_path>/profile.')
...
def load_inputs(args, MAX_IMG_W=1920, MAX_IMG_H=1080, profiler=None):
    ...
    if inputs_type == 'video':
        ...
        frames, video_meta = load_video(inputs_path, profiler=profiler)
        inputs_meta['fps'] = video_meta['fps']
        if profiler is not None:
            profiler.set_metadata(
                input_fps=video_meta['fps'],
                input_width=video_meta['w'],
                input_height=video_meta['h'],
                input_frames=video_meta['L'],
            )
        with profiler.measure('01_input.cpu_resize_frames') if profiler is not None else nullcontext():
        ...
        with profiler.measure('01_input.cpu_create_frame_views') if profiler is not None else nullcontext():
...
def _img_det2patches(
    img,
    det_instances,
    downsample_ratio: float,
    max_instances: int = 5,
    return_detection_meta: bool = False,
):
    '''
    ...
    if det_instances is None:  # no human detected
        empty = to_numpy([])
        if return_detection_meta:
            return empty, empty, {
                'scores': empty,
                'detector_bboxes_ltrb_px': empty,
            }
        return empty, empty
    ...
    detector_lurb_all = (
        det_instances['pred_boxes'][valid_mask].numpy() / downsample_ratio
    )  # (N, 4)
    detection_scores = det_instances['scores'][valid_mask].numpy()
    lurb_all = detector_lurb_all
    ...
    if return_detection_meta:
        return patches, cs_all, {
            ...
            'detector_bboxes_ltrb_px': detector_lurb_all,
        }
    return patches, cs_all
...
def prepare_mesh(
    ...
    batch_size: int = 16,
    profiler = None,
    return_body_data: bool = False,
) -> Tuple[Dict, Optional[Dict]]:
    # Full SKEL inference is memory consuming. A small batch also supports
    # 12 GB GPUs when processing long videos.
    v_skin_all, v_skel_all = [], []
    joints_44_all = []
    joints_24_anatomical_all = []
    joints_24_custom_all = []
    root_rotation_all = []
    for bw in asb(total=len(pd_params['poses']), bs_scope=batch_size, enable_tqdm=True):
        with profiler.measure('07_mesh.cpu_to_gpu_body_parameters', cuda=str(pipeline.device).startswith('cuda')) if profiler is not None else nullcontext():
            poses = pd_params['poses'][bw.sid:bw.eid].to(pipeline.device)
            betas = pd_params['betas'][bw.sid:bw.eid].to(pipeline.device)
        stage_name = (
            '07_mesh.gpu_skin_and_skeleton_shared_forward'
            if include_skeleton
            else '07_mesh.gpu_skin_forward'
        )
        with profiler.measure(stage_name, cuda=str(pipeline.device).startswith('cuda')) if profiler is not None else nullcontext():
            skel_outputs = pipeline.skel_model(
                    poses = poses,
                    betas = betas,
                    skelmesh = include_skeleton,
                )
        with profiler.measure('07_mesh.gpu_to_cpu_skin_vertices', cuda=str(pipeline.device).startswith('cuda')) if profiler is not None else nullcontext():
            v_skin = skel_outputs.skin_verts.detach().cpu()  # (B, Vi, 3)
        v_skin_all.append(v_skin)
        if include_skeleton:
            with profiler.measure('07_mesh.gpu_to_cpu_skeleton_vertices', cuda=str(pipeline.device).startswith('cuda')) if profiler is not None else nullcontext():
                v_skel = skel_outputs.skel_verts.detach().cpu()  # (B, Ve, 3)
            v_skel_all.append(v_skel)
        if return_body_data:
            with profiler.measure('07_mesh.gpu_to_cpu_3d_joints', cuda=str(pipeline.device).startswith('cuda')) if profiler is not None else nullcontext():
                joints_44_all.append(skel_outputs.joints.detach().cpu())
                joints_24_anatomical_all.append(
                    skel_outputs.joints_backup.detach().cpu()
                )
                joints_24_custom_all.append(
                    skel_outputs.joints_custom.detach().cpu()
                )
                # joints_ori[:, 0] contains the posed root-bone orientation,
                # including SKEL's fixed root-bone rest alignment. Removing
                # that rest alignment gives the exact global root rotation
                # that moved the canonical body into the model camera axes.
                root_rest_orientation = pipeline.skel_model.per_joint_rot[0]
                root_rotation = torch.matmul(
                    skel_outputs.joints_ori[:, 0],
                    root_rest_orientation.transpose(0, 1),
                )
                root_rotation_all.append(root_rotation.detach().cpu())
    with profiler.measure('07_mesh.cpu_concatenate_vertices') if profiler is not None else nullcontext():
    ...
    if include_skeleton:
        ...
        m_skel = None
    if not return_body_data:
        return m_skin, m_skel

    body_data = {
        'joints_44': torch.cat(joints_44_all, dim=0),
        'joints_24_anatomical': torch.cat(
            joints_24_anatomical_all,
            dim=0,
        ),
        'joints_24_custom': torch.cat(joints_24_custom_all, dim=0),
        'root_rotation': torch.cat(root_rotation_all, dim=0),
    }
    return m_skin, m_skel, body_data
...
def visualize_full_img(pd_cam_t, raw_imgs, det_meta, m_skin, m_skel, have_caption:bool=False, profiler=None) -> Tuple[List[np.ndarray], np.ndarray]:
    ''' Render the results to the patches. '''
    ...
    for i in tqdm(range(len(raw_imgs)), desc='Rendering'):
        ...
        with profiler.measure('08_render.cpu_camera_transform') if profiler is not None else nullcontext():
        ...
        for view in ['front']:  # ['front', 'side60d', 'side90d', 'top90d']
            with profiler.measure('08_render.opengl_skin_mesh') if profiler is not None else nullcontext():
            ...
            if m_skel is not None:
                with profiler.measure('08_render.opengl_skeleton_mesh') if profiler is not None else nullcontext():
            ...
            with profiler.measure('08_render.cpu_blend_and_splice') if profiler is not None else nullcontext():
                if m_skel is not None:
