...
def load_video(
    ...
    if video_path.is_dir():
        ...
        if profiler is not None:
            profiler.add('01_input.cpu_image_sequence_read_decode', time.perf_counter() - started_at)
        fps = 30 # default fps
    else:
        ...
        if profiler is not None:
            profiler.add('01_input.cpu_video_open_count_metadata', time.perf_counter() - started_at)
        frames = []
        started_at = time.perf_counter()
        for frame in tqdm(reader, total=frame_count):
            frames.append(frame)
        if profiler is not None:
            profiler.add('01_input.cpu_compressed_read_and_decode', time.perf_counter() - started_at)
        reader.close()
    started_at = time.perf_counter()
    frames = np.stack(frames, axis=0) # (L, H, W, 3)
    if profiler is not None:
        profiler.add('01_input.cpu_stack_decoded_frames', time.perf_counter() - started_at)
    meta = {
        'fps': fps,
        'w'  : frames.shape[2],
        'h'  : frames.shape[1],
...
def save_video(
    ...
    if isinstance(frames, List):
        ...
        if profiler is not None:
            profiler.add('09_output.cpu_stack_rendered_frames', time.perf_counter() - started_at)
    ...
    if resize_ratio is not None:
        ...
        if profiler is not None:
            profiler.add('09_output.cpu_resize_rendered_frames', time.perf_counter() - started_at)
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)

    started_at = time.perf_counter()
    writer = imageio.get_writer(output_path, fps=fps, quality=quality)
    if profiler is not None:
        profiler.add('09_output.cpu_video_writer_open', time.perf_counter() - started_at)
    output_seq_name = str(output_path).split('/')[-1]
    started_at = time.perf_counter()
    for frame in tqdm(frames, desc=f'Saving {output_seq_name}'):
        writer.append_data(frame)
    if profiler is not None:
        profiler.add('09_output.cpu_video_encode_and_write', time.perf_counter() - started_at)
    started_at = time.perf_counter()
    writer.close()
    if profiler is not None:
        profiler.add('09_output.cpu_video_writer_flush_close', time.perf_counter() - started_at)
